    ...
    let cnt = [
      div [_class "container "] [
        ...
        ul [] [
          ...
          li [] [ strong [] [rawText "Title: "]; rawText o.title ]
          li [] [ strong [] [rawText "Author: "]; rawText o.author ]
        ]
        a [_class "button is-text"; _href (Links.edit ctx o.id)] [rawText "Edit"]
    ...
    let cnt = [
      div [_class "container "] [
        form [ _action (if isUpdate then Links.withId ctx o.Value.id else Links.index ctx ); _method "post"] [
          ...
          yield field (fun i -> i.title) "Title" "title" 
          yield field (fun i -> i.author) "Author" "author" 
          yield buttons
        ]
      ]
    ]
